...
namespace Emp37.Utility.Tween
{
      public static class Ease
      {
            ...
            private static float EaseInElastic(float x, float overshoot)
            {
                  ...
                  return Pow(2F, 10F * x) * Sin((x - Elastic) * Elastic) * overshoot;
            }
            private static float EaseOutElastic(float x, float overshoot)
            {
                  if (x == 0F) return 0F; else if (x == 1F) return 1F;
                  ...
                  return 1F + Pow(2F, -10F * x) * Sin((x - 0.075F) * Elastic) * overshoot;
            }
            private static float EaseInOutElastic(float x, float overshoot)
            {
                  if (x == 0) return 0F; else if (x == 1F) return 1F;
